Shader "Lightweight Render Pipeline/2D/DistortShader"
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 displace = float2(sin(i. / 30 + _Time[3]) / 50, 0);
                float4 mainTex = i.color * SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, i.uv + displace);
                return mainTex;
            }
            ENDHLSL
        }
